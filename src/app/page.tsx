...
import { MessageCircle, Phone, ArrowRight, Truck, Star, Package, Factory, Cog, Network, Warehouse, Container, ShieldCheck, Building2, MapPin, Percent } from 'lucide-react'
...
function TimberSizeCard({ v }: { v: typeof PRODUCT_VARIANTS[0] }) {
  ...
  return (
    ...
    >
      <div className="text-sm md:text-lg font-bold text-primary-600">{formatVariantLabel(v)}</div>
      ...
      {v.price && <div className="text-[11px] font-bold text-primary-700 dark:text-primary-400 mb-1">{formatTZS(v.price)}</div>}
      <div className="flex items-center justify-between pt-1 border-t border-gray-100 dark:border-gray-700">
        <StockBadge v={v} />
        <span className="text-[10px] font-semibold text-primary-600 group-hover:underline">View</span>
      </div>
    </Link>
  )
}
...
const TRUST_COUNTERS = [
  { value: '1,500+', label: 'Projects Supplied', icon: Building2 },
  { value: '50+', label: 'Supply Partners', icon: Network },
  { value: '11', label: 'Island-wide Locations', icon: MapPin },
  { value: '<30 min', label: 'Fast Quotations', icon: Star },
]

const FEATURED_VARIANTS = PRODUCT_VARIANTS.filter(v => v.price).slice(0, 6)
  ...
  return (
    <>
      ...
      <main>
        ...
        {/* 2. Why Zanzibaba? — Construction Supply Network */}
            ...
            <div className="max-w-5xl mx-auto">
              <div className="flex flex-col md:flex-row items-center gap-4 md:gap-8 text-center md:text-left mb-5">
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <div>
                    <h2 className="text-base md:text-lg font-bold">Construction Supply Network</h2>
                    <p className="text-primary-200 text-xs">Not just a timber yard</p>
                  </div>
                </div>
                <p className="text-sm text-primary-100 leading-relaxed">
                  Zanzibaba Timber is a fully integrated construction supply network with our own treatment plant, kiln drying, stock yard, and delivery fleet — plus strategic partnerships across mainland Tanzania. We supply contractors, hotels, government projects, and villa developments across all Zanzibar.
                </p>
              </div>

              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                {TRUST_COUNTERS.map((c) => (
                  <div key={c.label} className="bg-white/10 rounded-lg p-3 md:p-4 text-center backdrop-blur-sm">
                    <c.icon className="w-5 h-5 text-green-400 mx-auto mb-1" />
                    <div className="text-lg md:text-2xl font-bold">{c.value}</div>
        ...
        {/* 3. Premium Timber Sizes — first 6 with prices */}
        ...
        {/* 5. Testimonials — moved above quote */}
        ...
        {/* 6. Quote Form — after social proof */}
        ...
        {/* 8. Why Zanzibaba Timber — detailed advantages */}
        ...
        {/* 9. How It Works — 2-col on mobile */}
            ...
            <div className="max-w-4xl mx-auto space-y-2">
              {HOMEPAGE_FAQ.slice(0, 3).map((faq, i) => (
                <details key={i} className="group bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                  ...
                  <div className="px-3 pb-3 text-gray-600 dark:text-gray-300 border-t border-gray-200 dark:border-gray-700 pt-2 text-xs md:text-sm">
                    {faq.answer}
                  </div>
                </details>
              ))}
